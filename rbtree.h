...
public:
    ...
    void insert(const iterator &it, const std::pair<K, V> &key_value) {
        ...
        // TODO
        V val = key_value.second;

        RedBlackNode<K, V> *node = new RedBlackNode<K, V>(key, val);
        if (root_ != NULL && check_dupes(root_, node)) {
            std::stringstream iss;
            iss << key;
            std::string newKey = iss.str();
            delete node;
            tree_exception e("Attempt to insert duplicate key '" + newKey + "'.");
            throw e;
        }
    ...
    /**
     * Checks for duplicated given a root node and a node to check
     */
    bool check_dupes(Node<K, V> *root, Node<K, V> *node){
        if (root == NULL){
            return false;
        };
        ...
        return (check_dupes(root->right(), node) or check_dupes(root->left(), node));
    }
...
private:
    ...
    void insert_fixup(RedBlackNode<K, V> *z) {
        ...
        if ((par->color() == RED &&
            ((par->left() != NULL &&
              par->left()->color() == RED) ||
            (par->right() != NULL && par->right()->color() == RED))) || 
            root_->color() != BLACK){
            if (par->parent() == NULL){
            ...
            //setting uncle
            RedBlackNode<K, V> *gp = par->parent(), *unc;
            if (par == gp->left()){
                unc = gp->right();
            } else {
                unc = gp->left();
            ...
            if (unc != NULL && unc->color() == RED){
                ...
                if ((unc == NULL || (unc != NULL && unc->color() == BLACK)) &&
                    par->right() == z){
                    z = par;
                    left_rotate(z);
                    insert_fixup(z);
                } else if ((unc == NULL || (unc != NULL && unc->color() == BLACK)) &&
                           par->left() == z){
                    par->set_color(BLACK);
                    gp->set_color(RED);
                    right_rotate(gp);
    ...
    void left_rotate(Node<K, V> *x) {
        RedBlackNode<K, V> *cur_right = static_cast<RedBlackNode<K, V>*>(x->right());
        x->set_right(cur_right->left());
        
        if(cur_right->left() != NULL){
            cur_right->left()->set_parent(x);
        }
        cur_right->set_parent(x->parent());

        if (x->parent() == NULL){
            root_ = cur_right;
        } else if (x == (x->parent()->left())){
            x->parent()->set_left(cur_right);
        } else {
            x->parent()->set_right(cur_right);
        }
        cur_right->set_left(x);
        x->set_parent(cur_right);
    }
    ...
    void right_rotate(Node<K, V> *x) {
        RedBlackNode<K, V> *cur_left = static_cast<RedBlackNode<K, V>*>(x->left());
        x->set_left(cur_left->right());
        if (cur_left->right() != NULL){
            cur_left->right()->set_parent(x);
        }
        cur_left->set_parent(x->parent());
        if (x->parent() == NULL){
            root_ = cur_left;
        } else if (x == (x->parent()->right())){
            x->parent()->set_right(cur_left);
        } else {
            x->parent()->set_left(cur_left);
        }
        cur_left->set_right(x);
        x->set_parent(cur_left);
    }
    ...
    size_t internal_node_count(Node<K, V> *node) const {
        ...
        Node<K, V> *tl = node->left();
        Node<K, V> *tr = node->right();
        if (node != NULL && tl == NULL && tr == NULL){
            return 0;
        } else if (tl == NULL && tr != NULL){
            return 1 + internal_node_count(tr);
        } else if (tl != NULL && tr == NULL){
            return 1 + internal_node_count(tl);
        } else {
